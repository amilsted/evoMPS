...
#            break
        ...
        if (not op is None) and (i % op_every == 0):
            ...
            data.append(map(lambda n: sfbc.Expect_SS(op, n).real, op_range))
...
class evoMPS_TDVP_Generic_FBC:
    ...
    def __init__(self, numsites, uGnd):
        ...
        self.uGndL.sanity_checks = self.sanity_checks
        ...
        self.uGndL.Calc_lr()
        self.uGndL.Calc_B()
        self.eta_uni = self.uGndL.eta
        
        self.uGndR = uni.evoMPS_TDVP_Uniform(uGnd.D, uGnd.q)
        self.uGndR.sanity_checks = self.sanity_checks
        ...
        for n in xrange(n_low, n_high):
            ...
            for u in xrange(self.q[n]):
                for v in xrange(self.q[n + 1]):
                    AA = mm.matmul(self.A[n][u], self.A[n + 1][v]) #only do this once for each 
    ...
    def CalcK(self, n_low=-1, n_high=-1):
        ...
        for n in reversed(xrange(n_low, n_high)):
            ...
            for s in xrange(self.q[n]): 
                for t in xrange(self.q[n+1]):
                    self.K[n] += mm.matmul(self.C[n][s, t], self.r[n + 1], 
                                           mm.H(self.A[n + 1][t]), 
                                           mm.H(self.A[n][s]))
                
                self.K[n] += mm.matmul(self.A[n][s], self.K[n + 1], 
                                       mm.H(self.A[n][s]))
        ...
        for n in reversed(xrange(-10000, 0)):
            ...
            for s in xrange(self.q[0]): 
                for t in xrange(self.q[0]):
                    K_n += mm.matmul(C[s, t], r_np1, mm.H(self.A[0][t]), 
                                     mm.H(self.A[0][s]))
                
                K_n += mm.matmul(self.A[0][s], K_np1, mm.H(self.A[0][s]))
    ...
    def BuildVsh(self, n, sqrt_r):
        ...
        for s in xrange(self.q[n]):
            R[:,s,:] = mm.matmul(sqrt_r, mm.H(self.A[n][s]))

        R = R.reshape((self.q[n] * self.D[n], self.D[n-1]))
        V = mm.H(ns.nullspace(mm.H(R)))
        
        if self.sanity_checks:
            if not sp.allclose(mm.matmul(V, R), 0):
                print "Sanity Fail in BuildVsh!: VR_%u != 0" % (n)
            if not sp.allclose(mm.matmul(V, mm.H(V)), sp.eye(V.shape[0])):
        ...
        if self.sanity_checks:
            ...
            for s in xrange(self.q[n]):
                M += mm.matmul(mm.H(Vsh[s]), sqrt_r, mm.H(self.A[n][s]))
    ...
    def CalcOpt_x(self, n, Vsh, sqrt_l, sqrt_r, sqrt_l_inv, sqrt_r_inv):
        ...
        for s in xrange(self.q[n]):
            ...
            if n < self.N + 1:
                x_subsubpart.fill(0)
                for t in xrange(self.q[n + 1]):
                    x_subsubpart += mm.matmul(self.C[n][s,t], self.r[n + 1], mm.H(self.A[n + 1][t])) #~1st line
                    
                x_subsubpart += mm.matmul(self.A[n][s], self.K[n + 1]) #~3rd line               
                
                x_subpart += mm.matmul(x_subsubpart, sqrt_r_inv)
            ...
            if not self.h_ext is None:
                ...
                x_subpart += mm.matmul(x_subsubpart, sqrt_r)
            
            x_part += mm.matmul(x_subpart, Vsh[s])
                
        x += mm.matmul(sqrt_l, x_part)
        ...
        if n > 0:
            ...
            for s in xrange(self.q[n]):     #~2nd line
                x_subsubpart.fill(0)
                for t in xrange(self.q[n + 1]):
                    x_subsubpart += mm.matmul(mm.H(self.A[n - 1][t]), l_nm2, self.C[n - 1][t, s])
                x_part += mm.matmul(x_subsubpart, sqrt_r, Vsh[s])
            x += mm.matmul(sqrt_l_inv, x_part)
    ...
    def GetB(self, n):
        ...
        if self.q[n] * self.D[n] - self.D[n - 1] > 0:
            ...
            for s in xrange(self.q[n]):
                B[s] = mm.matmul(l_sqrt_inv, x, mm.H(Vsh[s]), r_sqrt_inv)
                
            if self.sanity_checks:
                M = sp.zeros_like(self.r[n - 1])
                for s in xrange(self.q[n]):
                    M += mm.matmul(B[s], self.r[n], mm.H(self.A[n][s]))
    ...
    def Get_l_r_roots(self, n):
        ...
        if self.sanity_checks:
            if not sp.allclose(mm.matmul(l_sqrt, l_sqrt), self.l[n - 1]):
                print "Sanity Fail in Get_l_r_roots: Bad l_sqrt_%u" % (n - 1)
            if not sp.allclose(mm.matmul(r_sqrt, r_sqrt), self.r[n]):
                print "Sanity Fail in Get_l_r_roots: Bad r_sqrt_%u" % (n)
            if not sp.allclose(mm.matmul(l_sqrt, l_sqrt_inv), sp.eye(l_sqrt.shape[0])):
                print "Sanity Fail in Get_l_r_roots: Bad l_sqrt_inv_%u" % (n - 1)
            if not sp.allclose(mm.matmul(r_sqrt, r_sqrt_inv), sp.eye(r_sqrt.shape[0])):
    ...
    def Upd_l(self, start=-1, finish=-1):
        ...
        for n in xrange(start, finish + 1):
            ...
            for s in xrange(self.q[n]):
                self.l[n] += mm.matmul(mm.H(self.A[n][s]), self.l[n - 1], self.A[n][s])
    ...
    def EpsR(self, res, n, x, o):
        ...
        if o is None:
            for s in xrange(self.q[n]):
                res += mm.matmul(self.A[n][s], x, mm.H(self.A[n][s]))            
        else:
            for s in xrange(self.q[n]):
                for t in xrange(self.q[n]):
                    o_st = o(n, s, t)
                    if o_st != 0.:
                        tmp = mm.matmul(self.A[n][t], x, mm.H(self.A[n][s]))
                        tmp *= o_st
                        res += tmp
        return res
    ...
    def EpsL(self, res, n, x):
        ...
        for s in xrange(self.q[n]):
            res += mm.matmul(mm.H(self.A[n][s]), x, self.A[n][s])
        return res
    ...
    def Restore_ON_R_n(self, n, G_n_i):
        ...
        if G_n_i is None:
            ...
            GGh_n_i = mm.matmul(G_n_i, self.r[n], mm.H(G_n_i))
        ...
        try:
            ...
            e,Gh = la.eigh(M)
            G_nm1 = mm.H(mm.matmul(Gh, sp.diag(1/sp.sqrt(e) + 0.j)))
        ...
        for s in xrange(self.q[n]):                
            self.A[n][s] = mm.matmul(G_nm1, self.A[n][s], G_n_i)
    ...
    def Restore_RCF_r(self):
        ...
        for s in xrange(self.q[0]): #Note: This does not change the scale of A[0]
            self.A[0][s] = mm.matmul(G_n, self.A[0][s], G_n_i)            
            
        self.uGndL.r = mm.matmul(G_n, self.uGndL.r, mm.H(G_n))
        self.l[0] = mm.matmul(mm.H(G_n_i), self.l[0], G_n_i)
    ...
    def Restore_RCF_l(self):
        ...
        for n in xrange(self.N + 1):
            if n == 0:
                ...
                x = mm.matmul(mm.H(G_nm1), l_nm1, G_nm1)
            ...
            if n == 0:
                ...
                self.uGndL.r = mm.matmul(G_nm1, self.uGndL.r, G_n_i) #since r is not eye
        ...
        for s in xrange(self.q[self.N + 1]):
            self.A[self.N + 1][s] = mm.matmul(G_nm1, self.A[self.N + 1][s], G_nm1_i)
            
        ##This should not be necessary if G_N is really unitary
        #self.r[self.N] = mm.matmul(G_nm1, self.r[self.N], mm.H(G_nm1))
        #self.r[self.N + 1] = self.r[self.N]
        self.uGndR.l[:] = mm.matmul(mm.H(G_nm1_i), self.uGndR.l, G_nm1_i)
    ...
    def DensityMatrix_2S(self, n1, n2):
        ...
        for s2 in xrange(self.q[n2]):
            for t2 in xrange(self.q[n2]):
                r_n2 = mm.matmul(self.A[n2][t2], self.r[n2], mm.H(self.A[n2][s2]))
                ...
                for s1 in xrange(self.q[n1]):
                    for t1 in xrange(self.q[n1]):
                        r_n1 = mm.matmul(self.A[n1][t1], r_n, mm.H(self.A[n1][s1]))
                        tmp = mm.matmul(self.l[n1 - 1], r_n1)
                        rho[s1 * self.q[n1] + s2, t1 * self.q[n1] + t2] = tmp.trace()
        return rho
    
    def ApplyOp_SS(self, o, n):
